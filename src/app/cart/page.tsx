...
function Page() {
  ...
  return (
    ...
    <div className="cart min-h-screen ">
      ...
      <div className="header flex justify-end pe-10 my-4">
        <Button onClick={ClearCart}>
          Clear Cart <Trash color="red" />
        </Button>
      </div>
      {!cart ? (
        <div className="cart-skeleton md:flex w-90/100 mx-auto gap-4 items-start">
          <div className="elements border rounded-2xl md:w-60/100">
            {[...Array(3)].map((_, idx) => (
              <Skeleton key={idx} className="h-32 w-full mb-4" />
            ))}
          </div>
